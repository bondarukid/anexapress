/* eslint-disable @next/next/no-img-element -- Safari mockup screen uses img like shadcnblocks bundle */
...
const SCREEN_LEFT = `${(OFFSET_X / WIDTH) * 100}%`;
const SCREEN_TOP = `${(BAR_HEIGHT / HEIGHT) * 100}%`;
const SCREEN_WIDTH_PCT = `${(SCREEN_WIDTH / WIDTH) * 100}%`;
const SCREEN_HEIGHT_PCT = `${(SCREEN_HEIGHT / HEIGHT) * 100}%`;

const SCREEN_CLIP_PATH =
  "M1 52H1201V741C1201 747.075 1196.08 752 1190 752H12C5.92486 752 1 747.075 1 741V52Z";
...
function SafariDefaultNavIcons() {
  return (
    <>
...
/**
 * Safari browser mockup frame (shadcnblocks feature299 / magicui pattern).
 */
export function Safari({
  ...
  const id = useId();
  const punchMaskId = `safariPunch-${id}`;
  const clipPathId = `path0-${id}`;
    ...
    >
      ...
      {!hasVideo && imageSrc ? (
        <div
          ...
          style={{ ...screenStyle, borderRadius: "0 0 11px 11px" }}
        >
      ...
      >
        <defs>
          <mask id={punchMaskId} maskUnits="userSpaceOnUse">
            ...
            <path d={SCREEN_CLIP_PATH} fill="black" />
          </mask>
          <clipPath id={clipPathId}>
            <rect width={WIDTH} height={HEIGHT} fill="white" />
          </clipPath>
          <clipPath id={`roundedBottom-${id}`}>
            <path d={SCREEN_CLIP_PATH} fill="white" />
          </clipPath>
        </defs>

        <g clipPath={`url(#${clipPathId})`} mask={hasMedia ? `url(#${punchMaskId})` : undefined}>
          <path
          ...
          {mode === "default" ? <SafariDefaultNavIcons /> : null}
        </g>
      </svg>
    </div>
  );
